...
# Monkeypatch beartype in export_flow_lm as well for consistency
import pocket_tts.models.tts_model
import pocket_tts.models.flow_lm
import pocket_tts.models.mimi
...
def patched_conv1d_forward(self, x, model_state: dict | None):
    ...
    if TP and self.pad_mode == "replicate":
        ...
        init = x[..., :1]
        # Out-of-place update
        new_prev = torch.where(state["first"].view(-1, 1, 1), init, state["previous"])
...
from onnx_export.wrappers import (
    FlowLMWrapper,
    MimiWrapper,
    MimiEncoderWrapper,
    TextConditionerWrapper,
)
from pocket_tts.modules import conv
import math

# Mimi decoder state length is measured in 200 Hz decoder steps, not text tokens.
# 512 steps gives room for 32 latent frames, or about 2.56 seconds of audio.
# Longer generations are handled by the inference code via chunking/streaming.
MIMI_STATIC_SEQ_LEN = 512
...
def export_models(
    output_dir="onnx_models", weights_path="weights/tts_b6369a24.safetensors"
):
    ...
    mimi_encoder_wrapper = MimiEncoderWrapper(
        tts_model.mimi, speaker_proj_weight=tts_model.flow_lm.speaker_proj_weight
    )
    ...
    mimi_state = init_states(
        tts_model.mimi, batch_size=1, sequence_length=MIMI_STATIC_SEQ_LEN
    )
    ...
    mimi_output_names = ["audio_frame"] + [
        f"out_state_{i}" for i in range(len(flat_mimi_state))
    ]

    # Mimi dynamic axes
    mimi_dynamic_axes = {"latent": {1: "seq_len"}}
...
def verify_export(flow_lm_path, mimi_path, tts_model, output_dir="onnx_models"):
    ...
    if os.path.exists(encoder_path):
        ...
        np.testing.assert_allclose(
            pt_encoder_out.numpy(), onnx_encoder_out, rtol=1e-4, atol=1e-4
        )
    ...
    if os.path.exists(conditioner_path):
        ...
        np.testing.assert_allclose(
            pt_conditioner_out.numpy(), onnx_conditioner_out, rtol=1e-5, atol=1e-5
        )
    ...
    if mimi_path and os.path.exists(mimi_path):
        ...
        # ONNX run
        ort_mimi_inputs = {"latent": latent.numpy()}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export Mimi and Conditioner models to ONNX."
    )
    parser.add_argument(
        "--output_dir",
        "-o",
    ...
    flow, mimi, model = export_models(
        output_dir=args.output_dir, weights_path=args.weights_path
    )
